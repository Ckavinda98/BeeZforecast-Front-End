...
import { Saveprediction } from "../Action/result";

const PredictForm = () => {
  const [state, setStates] = useState("");
  const [varroa_mites, setVarroam] = useState("");
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await axios.post("http://localhost:8081/predict", data);
    const res = await Saveprediction(data);
  ...
  return (
    <>
      <div id="home" className="w-full h-screen bg-zinc-200 flex flex-col">
        <div className="grid md:grid-cols-2 max-w-[1240px] m-auto">
            ...
            border border-slate-300 rounded-md p-8 flex flex-col space-y-4 shadow-xl "
              >
                ...
                <label htmlFor="email" className="text-sm font-medium">
                  ...
                  parasites, such as wax moths, small hive beetles, and
                  non-Varroa mites.
                </label>
                <label htmlFor="email" className="text-sm font-medium">
                ...
                <label htmlFor="email" className="text-sm font-medium">
                  Pesticides Loss: Please enter the percentage of bee colony
                  loss due to exposure to pesticides.
                </label>
                <label htmlFor="email" className="text-sm font-medium">
                  Weather Loss: Please enter the percentage of bee colony loss
                  due to extreme weather conditions such as floods, droughts,
                  and hurricanes.
                </label>
                <label htmlFor="email" className="text-sm font-medium">
                  Queen Failure Loss: Please enter the percentage of bee colony
                  loss due to queen failure, which can result from poor
                  genetics, disease, or age.
                </label>
              </form>
            </div>
          </div>
          <div className="ml-2 px-2 py-6 mt-4">
            <form
              className="bg-zinc-300 border border-slate-300 rounded-md p-6 flex flex-col space-y-4 shadow-xl justify-center"
      ...
      {show && <Model onClose={onClose} result={result} />}
    </>
  );
};
